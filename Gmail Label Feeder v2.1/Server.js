// By mhawksey based on work by Eric Koleda

/*

==Setup instructions==
1. File > Make a copy of this script
1. Run > setup IMPORTANT: when authenticating use the Gmail account you want to create feeds for
1. File > Manage versions... and create an initial name and 'Save New Version'
1. Deploy > New deployment Deploy as web app... and select web app and 'Execute the app as: me' and allow anyone access even anonymously
1. Open (and bookmark) the 'current web app url' to create (atom) feeds for your gmail labels

See http://mashe.hawksey.info/2013/05/gmail-label-to-rss-feed for more info
...
function doGet(event) { 
  ...
  if (!xmlString) {
    if (!messageid){
      ...
      var xmlString = convertActivitiesToAtom_(threads, feed.input, selfUrl, feed.exclude);
    } else {
 ...
 * @param {string} exclude The link text to exclude.
 * @returns {Array} A shorthand XML Array of the ATOM feed.
 */
function convertActivitiesToAtom_(threads, name, selfUrl, exclude) { 
  var rss = makeRss();

  rss.setTitle(name);
  rss.setLink(selfUrl);
  rss.setDescription('Gmail Label Feeder v2.1');
  rss.setLanguage('en');
  rss.setAtomlink(selfUrl);
  rss.setUpdated(new Date);
  
  _.each(threads, function(item) {
    var message = item.getMessages()[0]; // get first message
    rss.addItem({
      title: item.getFirstMessageSubject(),
      guid: selfUrl + "&messageid="+message.getId(),
      link: selfUrl + "&messageid="+message.getId(),
      description: cleanBody(item.getMessages()[0].getBody(), exclude),
...
function cleanBody(html, exclude){
  html = html.replace(/\r?\n|\r|\t/g, '');
  var re="<a\\b[^>]*>(<[^>]+>)*("+exclude+")(<(?!/a>)[^>]+>)*</a>"; // http://stackoverflow.com/a/21461843/1027723
